...
import { EventResponse, Message } from '@/types/app';
...
export default function ConversationScreen() {
  ...
  const { conversations, messages, sendMessage, createPoll, createEvent, votePoll, respondToEvent } = useApp();
  ...
  const chooseAttachment = (label: string) => {
    setShowAttachments(false);
    if (label === 'Poll' || label === 'Event') {
      setActiveComposer(label.toLowerCase() as 'poll' | 'event');
    } else {
      Alert.alert(label, `${label} messages are prepared for device integration in the next local build.`);
    }
  };
  ...
  return <KeyboardAvoidingView style={[styles.root, { backgroundColor: colors.background }]} behavior="padding">
    ...
    <FlatList data={items} keyExtractor={(item) => item.id} contentContainerStyle={styles.messages} keyboardDismissMode="interactive" keyboardShouldPersistTaps="handled" ListHeaderComponent={<><PressableScale onPress={() => Alert.alert('Earlier messages', 'This local preview keeps the most recent messages on-device.')} style={[styles.loadEarlier, { borderColor: colors.border }]}><Text style={[styles.loadEarlierText, { color: colors.primary }]}>Load earlier messages</Text></PressableScale><View style={styles.dateRow}><View style={[styles.dateRule, { backgroundColor: colors.border }]} /><Text style={[styles.date, { color: colors.mutedForeground }]}>Today</Text><View style={[styles.dateRule, { backgroundColor: colors.border }]} /></View></>} ListEmptyComponent={<Text style={[styles.empty, { color: colors.mutedForeground }]}>No messages match “{query}”.</Text>} renderItem={({ item }) => { const sender = staff.find((person) => person.id === item.senderId); const senderName = item.outgoing ? 'You' : sender?.name ?? 'Team member'; return <View style={[styles.messageRow, item.outgoing && styles.outgoing]}><Text style={[styles.senderName, { color: item.outgoing ? colors.mutedForeground : getSenderColor(item.senderId, isDark) }]}>{senderName}</Text><PressableScale onLongPress={() => openActions(item.text)}><View style={[styles.bubble, item.outgoing ? styles.outgoingBubble : styles.incomingBubble, { backgroundColor: item.attachment ? colors.bubbleIncoming : item.outgoing ? colors.royalBlue : colors.bubbleIncoming }]}>{renderAttachment(item)}<View style={styles.messageMeta}><Text style={[styles.messageTime, { color: item.attachment || !item.outgoing ? colors.mutedForeground : 'rgba(255,255,255,0.78)' }]}>{item.time}</Text>{item.outgoing ? <Ionicons name={item.status === 'read' ? 'checkmark-done' : 'checkmark'} size={14} color={item.attachment ? colors.mutedForeground : item.status === 'read' ? colors.statusDone : 'rgba(255,255,255,0.78)'} /> : null}</View></View></PressableScale></View>; }} />
